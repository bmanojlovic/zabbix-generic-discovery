...
if len(sys.argv) < 4:
    print(
        ...
        "    2) What is name of variable that should be"
        " replaced in discovery\n"
        "    3) Replacement value\n"
        "  "
    )
    sys.exit()
...
def graph_item_get(hostid, itemid, PLACEHOLDER, REPLACEMENT):
    ...
    """
    origitemproto = (
        zapi.itemprototype.get(hostids=hostid, itemids=itemid,
                               output="extend", selectApplications={})
    )[0]
    origitemproto['key_'] = origitemproto['key_'].replace(
        PLACEHOLDER, REPLACEMENT)
    fixed_itemid = zapi.itemprototype.get(
        hostids=hostid, search={'key_': origitemproto['key_']})
    if len(fixed_itemid) > 0:
        print("found itemprototype with key = %s" % origitemproto["key_"])
    return fixed_itemid[0]


for drule in zapi.discoveryrule.get(
        hostids=hostid, selectItems={},
        selectGraphs={}, selectTriggers={},
        selectHostPrototypes={}, search={"key_": PLACEHOLDER}):
    """ Create discovery rule based on template """
    drule_c = zapi.discoveryrule.get(itemids=drule['itemid'],
                                     output="extend")[0]
    ...
    del drule_c['state']
    drule_created = zapi.discoveryrule.get(
        hostids=hostid, search={"key_": drule_c["key_"]})
    ...
    for itemprotoid in drule['items']:
        itemproto = (
            zapi.itemprototype.get(
                hostids=hostid, itemids=itemprotoid,
                output="extend", selectApplications={}
            )
        )[0]
        ...
        if len(zapi.itemprototype.get(
                hostids=hostid,
                search={'key_': itemproto['key_']})) == 0:
    ...
    for triggerprotoid in drule['triggers']:
        triggerproto = (zapi.triggerprototype.get(
            triggerids=triggerprotoid, output="extend",
            expandExpression="true"))[0]
        del triggerproto['templateid']
        del triggerproto['triggerid']
        triggerproto['description'] = triggerproto['description'].replace(
            PLACEHOLDER, REPLACEMENT)
        triggerproto['expression'] = triggerproto['expression'].replace(
            PLACEHOLDER, REPLACEMENT)
        try:
            ...
            print("created triggerproto"
                  " with expression = %s" % triggerproto["expression"])

    """ Create graphs on created prototype items """
    for graphprotoid in drule['graphs']:
        graphprotoitems = (
            zapi.graphitem.get(graphids=graphprotoid, selectItems={},
                               selectGraphItems={}, output="extend")
        )
        gitems = []
        for gitem in graphprotoitems:
            gitem['itemid'] = graph_item_get(
                hostid, gitem['itemid'], PLACEHOLDER, REPLACEMENT)['itemid']
            del gitem['graphid']
            del gitem['gitemid']
            gitems.append(gitem)

        graphproto = zapi.graphprototype.get(
            hostids=hostid, graphids=graphprotoid, output="extend")[0]
        graphproto['name'] = graphproto['name'].replace(
            PLACEHOLDER, REPLACEMENT)
        del graphproto['graphid']
        del graphproto['templateid']
        graphproto['gitems'] = gitems
        if len(zapi.graphprototype.get(
                hostids=hostid, search={'name': graphproto['name']})) == 0:
